...
void QSqlConnectionDialog::writeSettings()
{
    ...
    settings.setValue("databasename", ui.editDatabase->text());
...
void QSqlConnectionDialog::readSettings()
{
    QSettings settings(this);
    settings.beginGroup("MainWindow");
    ui.editDatabase->setText(settings.value("databasename").toString());
    ui.editUsername->setText(settings.value("username").toString());
    ui.comboDriver->setCurrentText(settings.value("driver").toString());
    settings.endGroup();
}
